  ...
  <div class="col frame h-100 p-3" v-if="surah">
    <div class="col p-content pb-2 pt-3 mt-1 mb-4 text-justify">
      <template v-if="!status">
        <skeleton-loader
          v-if="surahPage.number === 1 || surahPage.number === 2"
          :lines="6"
        ></skeleton-loader>
      ...
      <template v-if="status">
        <div v-for="(sur, index) in surah" :key="index">
          <p
            class=""
            :class="{
              'frame-name-surah text-center': sur[0].numberInSurah === 1,
          ...
          <span class="p-0 " v-for="(ayah, index) in sur" :key="index">
            ...
            <template>
              <span
                :class="{ 'ayaha-active-color': ayah.number === ayahNumber }"
                v-html="
                  $options.filters.textColorNameAllah(
                    ayah.numberInSurah === 1
                      ? ayah.text.split(basmala)[1]
                      : ayah.text
                  )
                "
              >
              ...
              >
                ...
                <text
                  ...
                  dy="1.6em"
                >
                  {{ ayah.numberInSurah | ConvertToArabicNumbers }}
                </text>
                <g id="header">
                  <path
                  ...
                  <use
                    xlink:href="#repu"
                    transform="translate(1248,0) scale(-1,1)"
                  />
                </g>
                <use
                  xlink:href="#header"
                  transform="translate(0,825) scale(1,-1)"
                />
              </svg>
            </template>
          </span>
      ...
      <div class="col text-center">
        <span
          ...
          class=" 
                "
          >{{ surahPage.number | ConvertToArabicNumbers }}</span
        >
      </div>
    </div>
  </div>
